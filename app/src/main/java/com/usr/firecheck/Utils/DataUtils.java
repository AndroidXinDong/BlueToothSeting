...
public class DataUtils {
    ...
    // 紫外电流系数
    public static String CMD_PURPLEECLECTRIC_CODE = "32";
    // 红外电流系数
    public static String CMD_REDPLEECLECTRIC_CODE = "33";
    // 可见光电流系数
    public static String CMD_LIGHTECLECTRIC_CODE = "34";
    // 前延时时长
    public static String CMD_BEFORE_CODE = "35";
    // 后延时时长
    public static String CMD_AFTER_CODE = "36";
    // 模式切换
...
/*****************************紫外电流系数、红外电流系数、可见光电流系数、前延时时长、后延时时长********************************/
    /**
     * 四字节浮点数转换的hex
     * @param hex
     * @return
     */
    public static String sendWriteCoef(String code,String hex){
        try {
            String temp = code + EXTEND_WRITE_CODE + "0004"+hex;
            byte[] bytes = Utils.hexStringToByteArray(temp);
            String crc = CRC.getCRC(bytes).replaceAll(" ","");
            String result = HEADER + temp + crc + TAIL;
            return result;
     ...
     * @return
     */
    public static String sendReadCoef(String code) {
        try {
            String temp = code + EXTEND_READ_CODE + "0000";
            byte[] bytes = Utils.hexStringToByteArray(temp);
            String crc = CRC.getCRC(bytes).replaceAll(" ", "");
            String result = HEADER + temp + crc + TAIL;
            return result;
     ...
     * @return
     */
    public static boolean getWriteCoefResponse(String code,String hex){
        try {
            String temp = code + EXTEND_WRITE_RESPONSE_CODE + "0000";
            byte[] bytes = Utils.hexStringToByteArray(temp);
            String crc = CRC.getCRC(bytes).replaceAll(" ","");
            String result = HEADER + temp + crc + TAIL;
            if (result.equals(hex)){
     ...
     *  12 - 20
     */
    public static int getReadCoefResponse(String hex){
        try {
            if (hex != null && hex.length() >=20) {
                String substring = hex.substring(12, 20);
                int decimal = (int) Float.parseFloat(C2JUtils.hex2Float(substring,4));
                return decimal;
            }
        } catch (Exception e) {
            e.printStackTrace();
